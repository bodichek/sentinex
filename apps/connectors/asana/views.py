...
def _redirect_uri(request: HttpRequest) -> str:
    return request.build_absolute_uri("/integrations/asana/callback/")
...
def connect(request: HttpRequest) -> HttpResponse:
    ...
    request.session["asana_oauth_state"] = state
    try:
...
def callback(request: HttpRequest) -> HttpResponse:
    ...
    expected = request.session.pop("asana_oauth_state", None)
    ...
    integration, _ = Integration.objects.get_or_create(provider="asana")
