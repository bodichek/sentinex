...
def ingest_customers(integration_id: int | None = None, mode: str = SyncMode.INCREMENTAL):
    return _run(FapiCustomerSync, integration_id, mode)


@shared_task(name="fapi.ingest.invoices")
def ingest_invoices(integration_id: int | None = None, mode: str = SyncMode.INCREMENTAL):
