...
def _redirect_uri(request: HttpRequest) -> str:
    return request.build_absolute_uri("/integrations/mailchimp/callback/")
...
def connect(request: HttpRequest) -> HttpResponse:
    ...
    request.session["mc_oauth_state"] = state
    try:
...
def callback(request: HttpRequest) -> HttpResponse:
    ...
    expected = request.session.pop("mc_oauth_state", None)
    ...
    integration, _ = Integration.objects.get_or_create(provider="mailchimp")
    integration.is_active = True
    integration.connected_at = timezone.now()  # type: ignore[assignment]
    integration.meta = {"dc": tokens.get("dc"), "api_endpoint": tokens.get("api_endpoint")}
