...
def _redirect_uri(request: HttpRequest) -> str:
    return request.build_absolute_uri("/integrations/microsoft365/callback/")
...
def callback(request: HttpRequest) -> HttpResponse:
    ...
    expected = request.session.pop("ms365_oauth_state", None)
    ...
    integration, _ = Integration.objects.get_or_create(provider="microsoft365")
    integration.is_active = True
    integration.connected_at = timezone.now()  # type: ignore[assignment]
    integration.meta = {"scope": tokens.get("scope")}
