...
def _redirect_uri(request: HttpRequest) -> str:
    return request.build_absolute_uri("/integrations/hubspot/callback/")
...
def connect(request: HttpRequest) -> HttpResponse:
    ...
    request.session["hs_oauth_state"] = state
    try:
...
def callback(request: HttpRequest) -> HttpResponse:
    ...
    expected = request.session.pop("hs_oauth_state", None)
    ...
    integration, _ = Integration.objects.get_or_create(provider="hubspot")
